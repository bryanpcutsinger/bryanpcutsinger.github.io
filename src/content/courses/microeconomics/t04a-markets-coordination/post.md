...
Step back and the coordination picture is coherent. Trade stops where the full price stops being worth paying, and intermediaries earn their cut by shrinking it. Open markets keep entry free; closing one transfers wealth to insiders without creating any. Above all, prices coordinate an economy no one is running, by carrying information, motivating action, and rationing scarce goods to those who value them most, even a supply that cannot grow. And we have drawn the crucial line between scarcity, a permanent fact of nature, and a shortage or surplus, which are not facts of nature at all but gaps that open only when a price is pinned below or above its clearing level.

That last distinction is the hinge into what comes next. If a shortage is what happens when a price is forbidden to rise, and a surplus is what happens when it is forbidden to fall, then we should look closely at the laws that do the forbidding: price ceilings and price floors. What exactly happens when a city caps rents or a government props up a wage? Where does the rationing go once money is barred from doing it? The next post, on price controls, picks up exactly here. The one after it turns to who really pays a tax when the thing taxed cannot get out of the way.
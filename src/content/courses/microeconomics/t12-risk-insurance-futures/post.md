...
<ul>
<li>Distinguish risk from uncertainty and distinguish an existing hazard and expected loss, which insurance cannot erase, from the spread one holder bears, which pooling and diversification can narrow; explain why the risk-return trade-off means no investment offers a higher return with a narrower spread and why a large holder self-insures.</li>
<li>Explain how insurance pools independent, measurable losses into a small, sure premium, why pooling makes insurance a positive-sum trade rather than a mere shift of risk, and state the five conditions that make a risk privately insurable.</li>
<li>Analyze how a government guarantee that breaks one or more of the five conditions stops pricing risk and becomes taxpayer-financed relief that breeds moral hazard, using deposit insurance as the case.</li>
</ul>
</aside>
...
That difference decides what markets can do. Because risk can be quantified, it can be exchanged between people, transformed from one kind into another, and hedged against; uncertainty cannot be quantified, so it cannot be traded or hedged. One shorthand says, "Risk behaves the way physicists say matter does: it is neither created nor destroyed." Read narrowly, that means transferring an existing hazard does not erase its expected loss: moving the exposure to a house fire does not change the chance of fire. Markets can still narrow the spread borne by each person through pooling. Pooling many independent exposures narrows that spread, while a casino bet deliberately creates a new exposure that did not exist before. When a market handles an existing risk well, it moves it to whoever will bear it most cheaply, pools it to reduce individual volatility, or both. One consequence runs against intuition: if a person voluntarily takes on more risk, that is not automatically bad. A gambler chooses a wider spread of outcomes on purpose, and so does an oil driller. Taking on risk is a choice, not a sin.
...
<h3>Key takeaways</h3>
<ul>
<li><strong>Markets transfer existing hazards and can narrow individual exposure.</strong> Insurance cannot erase the expected loss behind an existing hazard, but pooling and diversification can narrow the spread borne by one holder; no investment offers a higher return with a narrower spread, and a large holder self-insures.</li>
<li><strong>Insurance is a positive-sum trade.</strong> Pooling independent, measurable risks turns a small chance of ruin into a small, certain premium by narrowing each participant's exposure rather than erasing the underlying expected loss, which is why a risk is privately insurable only when the five conditions hold.</li>

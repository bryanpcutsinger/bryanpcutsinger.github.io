# Codex Context: Personal Website (bryanpcutsinger.github.io)
...
- `/ai/` — **Getting Started with Codex**: a beginner's guide to getting started with
  Codex, written for **academic economists** on a Mac who are new to the
...
- **Goal / guiding lens (2026-06-13):** the site exists to get a reader *self-sufficient
  ...
  plan: `~/.Codex/plans/crystalline-rolling-jellyfish.md`.
...
> Earlier history: `~/.Codex/plans/i-want-to-rebuilt-ancient-zephyr.md` (Rev 1–5) and
> `~/.Codex/plans/let-s-try-another-approach-async-knuth.md` (the 2026-07-01 rebuild).
...
- **Layout/kit:** `src/layouts/MarketingLayout.astro` (own `<head>`, sticky nav, footer,
  ...
  **`CitationList`** (bibliography; year folio). `Button` gained an **`on-ink`** variant.
  ...
  fee signal**, the **Codex guide as a hook**, a Recent list, email capture, and a
...
- **Theming = the "public economist" brand (retuned 2026-07-01; was FAU→brass→claret).**
  ...
  **ledger-green `#22453B`** accent; **Fraunces** headings, **Newsreader** prose, **Inter**
  ...
  Redesign plan: `~/.Codex/plans/let-s-try-another-approach-async-knuth.md`.
- **Content lives in `src/content/docs/`** (Starlight's `docs` collection, defined
  ...
  - `public/templates/Codex-md-economist-template.md` → `/templates/...` (a
    downloadable economist AGENTS.md starter, linked from `Codex-md-and-memory.md`)
...
- **Sidebar / curriculum order lives in the GUIDE repo** (`guide.manifest.json` in
  `bryanpcutsinger/Codex-guide`), NOT in `astro.config.mjs` anymore (changed
  2026-06-17 — see the "Codex guide" section below). The config just does
...
> fine. The `/ai/` Codex guide is now **imported** from `bryanpcutsinger/Codex-guide`
> (see the "Codex guide" section below): the overview (`ai/index.mdx`),
> `what-is-Codex`, `setting-up-Codex`, and `your-first-session` are
...
- **This repo is the ONLY publisher to the live site (single-publisher invariant).**
  ...
  carry a refusal **guardrail** in their own `AGENTS.md` (added 2026-06-20; committed +
  ...
  blocked there. (`Codex-guide` is also import-fed but does not yet carry this
  guardrail.)
...
- **A guide post:** author it in the **guide repo** (`~/Projects/Codex-guide`),
...
## Codex guide — imported from its own repo (split out 2026-06-17)

The `/ai/` "Getting Started with Codex" guide is **no longer hand-authored in
this repo**. Its source of truth is `bryanpcutsinger/Codex-guide` (private; local
folder `~/Projects/Codex-guide`, a top-level sibling of `Website`). This repo
holds a **generated, committed copy** — same source-of-truth model as the micro course,
but for Starlight content. Plan: `~/.Codex/plans/would-it-make-sense-humble-cupcake.md`.
...
- the guide's `public/**` → this repo's `public/**` (**additive** — never deletes
  ...
  `public/templates/Codex-md-economist-template.md` arrives).
...
- **Refresh ritual (when publications change):** the *list* updates itself from the CV
  ...
  paper's verbatim abstract in `research-abstracts.json`; (2) ask Codex to **"refresh the
...
blind two-leg review (Codex Plan agent + Codex gpt-5.6-sol), synthesized. Earlier
...
`~/.Codex/plans/let-s-try-another-approach-async-knuth.md` for the plan (which went through a
...
- **Recorded the single-publisher guardrail in the Deploy section.** Noted that this repo
  ...
  `AGENTS.md` (both committed + pushed) declining any live-deploy request and redirecting
...
- **Parallel micro work (`b7950ad`):** between the two Price Theory commits, a separate
  ...
  Heads-up: the AGENTS.md docs commit `2eea0bb` incidentally swept the T1 `post.md`/`html`
...
- **AI guide split into its own repo.** `/ai/` is now **imported** from
  `bryanpcutsinger/Codex-guide` (local `~/Projects/Codex-guide`); the
  website holds a generated, committed copy + a manifest-driven sidebar. Output is
  byte-identical to before the split. See "Codex guide — imported from its own
  repo" above.
...
- **The Cowork/Desktop branch was DELETED (2026-06-15).** A standalone "Codex on the
  ...
  **entirely on Codex**. Its sidebar group, the `sparkling-strolling-pearl*` plan
  files, and all `/ai/desktop/` references are gone. The marketing header/footer "Guides"
  link was repointed `/ai/desktop/` → `/ai/`, and the **`/ai/` Codex guide overview
  + `what-is-Codex` were published (`draft:false`)** so that link lands on real
...
- **Refocused as a lean "arrive-ready" pre-read** (plan:
  `~/.Codex/plans/crystalline-rolling-jellyfish.md`). A read-only multi-agent audit
  ...
  - **`your-first-session` gained a "Success looks like this" setup-verification**
    callout; `installing-Codex` now points to it as the end-of-setup check.
  - **Trims:** `what-is-Codex` "What it's NOT" compressed to one caution;
...
- **Phase 1 of the value-add audit is IMPLEMENTED** (plan:
  `~/.Codex/plans/velvet-conjuring-kernighan.md`). Grew the guide from 10 → **17
  posts** and added three sidebar stages. The seven new posts:
  `reading-and-analyzing`, `errors-and-recovery` (Working well);
  `teaching-with-Codex` (new **Teaching** stage); `a-real-research-workflow`,
  ...
  template link in `Codex-md-and-memory`; "backup ≠ version control" callout +
  recovery cross-link in `pointing-Codex`; author/credibility line + new stage
  cards on the overview; the `public/templates/` economist AGENTS.md asset; and the
  broken `/ai/getting-started/` link in `installing-vs-code.md` fixed to
  `/ai/what-is-Codex/`. All new pages verified 200 in `dev`.
...
- **PUBLISHED to GitHub Pages (2026-06-13).** Live at https://bryanpcutsinger.github.io
  ...
  The marketing site is the front end; under `/ai/`, the **Codex guide overview
  (`ai/index.mdx`) + `what-is-Codex`** are live (`draft:false`). Everything stays
  **`noindex`ed** while shared privately.
- **Mac/terminal Codex guide — mid-rebuild.** The old 17 posts were deleted in the
  blank-slate rebuild; the **overview + posts 1–3** (`what-is-Codex`,
  `setting-up-Codex`, `your-first-session`) are published (`draft:false`, in a
...
- **Screenshots — HARD pre-publish blockers on the required path.** Several posts have
  ...
  placeholder). Priority order: (1) `installing-Codex` — browser **sign-in page**
  (gates auth) + integrated-terminal panel; (2) `your-first-session` — VS Code with
  folder + Codex prompt (the "success looks like this" anchor); (3)
  `pointing-Codex-at-your-work` — side-by-side **diff** view; (4)
